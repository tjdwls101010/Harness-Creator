...
- A reference states the client behavior a model cannot observe at runtime, as the failure shape that changes a placement decision, and not the client-owned value behind it; a number the project itself chooses keeps its reason. `SKILL.md`'s authoring philosophy carries the rule, and `tools/claims.py` freezes what a file asserts so a rewrite has to account for each claim.
