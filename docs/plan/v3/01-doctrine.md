...
**C14 — Operating loop의 코드펜스(548단어, 파일의 21%)를 아무도 측정하지 않았다.** 정본 측정 스크립트가 ```` ``` ```` 펜스를 제거하고 시작하므로 장문단 통계에 한 번도 잡힌 적이 없다. 실제로는 트리 주석 안에 파일에서 가장 밀도 높은 논증이 들어 있었고, Wrap-up의 `validate_harness.py` 주석 하나에서만 29단어가 나왔다. **PR2도 같은 사각지대를 갖는다** — `hooks.md`·`workflows.md`·`e2e-testing.md`는 코드펜스 비중이 크다. 각 파일에서 펜스 안 산문 주석을 별도로 훑는다.
