...
### M4. 도그푸딩 (성공 기준 #5)
- 샘플 대상: 간단한 실제 프로젝트 1개(사용자가 지정하거나, 없으면 작은 오픈소스 클론).
- 전체 루프 실행: 감사 → 인터뷰(사용자 참여) → 스펙 → 생성 → validate 0오류 → test_hook →
  (동의 시) e2e 2~3 시나리오.
- **재진입 검증**: 같은 프로젝트에 harness-creator를 한 번 더 호출해 확장 또는 개선 모드로
  분기하는지, 중복 생성이 없는지 확인 (성공 기준 3은 이 단계가 검증한다).
- 발견된 문제를 스킬에 반영(gotcha 축적 — 개선 독트린의 첫 실천).
- 수용: 00 문서 §4 성공 기준 1~3, 5 충족.
