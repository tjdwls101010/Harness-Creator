...
5. **Wrap-up** — a change-history entry in the spec, any needed `CLAUDE.md` pointer updates, one final whole-harness `validate_harness.py` pass (run last on purpose: the spec and `CLAUDE.md` edits in the two previous steps are themselves the kind of thing the linter checks — an `@import`, a 200-line overage, a bare-name inventory listing — so this pass catches mistakes those edits could have introduced), and a proposed commit.
