...
| Piece | Loaded | Role |
| --- | --- | --- |
| `SKILL.md` | Every invocation | The orchestrator — philosophy, the layer-routing framework, the operating loop, script usage summaries. Kept under the skill's own 3,750-word ceiling (currently ~2,400 words). |
| `references/*.md` | On demand, per component or per mode | Nine authoring guides (CLAUDE.md & rules, skills, hooks, hook events, agents, workflows, interview, e2e testing, re-entry) — together far larger than `SKILL.md`. A session that only writes a `CLAUDE.md` never loads the hook-events table. |
...
The split follows the branch "which component am I generating right now," which is exactly the point at which the matching reference becomes relevant. `interview.md` and `re-entry.md` split along a second, orthogonal branch — not component type but *invocation shape*: a brand-new build loads only `interview.md`, `extend`/`improve` load `re-entry.md` first and then `interview.md` for the stages it re-enters, and `sync` loads `re-entry.md` alone and never opens `interview.md` at all. That gate is what keeps the always-loaded surface (`SKILL.md` plus whatever Phase 0 unconditionally reads) small — earlier revisions loaded `interview.md` unconditionally on every invocation, which doubled the effective always-loaded cost; see [`docs/plan/v2/00-overview.md`](../plan/v2/00-overview.md) §3.1b for the measurement. What is deliberately *not* split further: per-component templates stay inline in each guide rather than in a separate `templates/` directory that would only add routing cost.
