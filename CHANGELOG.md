...
- **What a reference may assert, in `SKILL.md`'s authoring philosophy.** A reference carries the client behavior the model cannot observe while it runs, written as the failure shape that changes a placement decision, and not the client-owned value behind it — a release moves the value while the prose goes on asserting the old one. A number the project itself chooses is the opposite case and keeps its reason, which is what the replaced "Numbers need a reason" sentence had covered. Alongside it, the line written against a model default: the most useful line in a harness and the one whose expiry is tied to the model, so it records which default it corrects and a model change reopens it rather than retiring it.
- **`references/claude-md-and-rules.md` states failure shapes where it used to state client-owned values.** The `~200 lines` target is gone, and with it the monorepo exception that existed only to except a number; what remains is the per-line test that the target had been answering for the whole file at once, with length named as the symptom it diagnoses rather than a budget to meet. The memory index's line and byte limits, and the import depth cap, are likewise replaced by what a harness author actually decides from: an entry past the index's cut is invisible to the session reading it, and an import chain built past the depth delivers nothing from its tail. The pointer policy is rule-first in two paragraphs instead of three. Two additions the audit turned up on the way: routing a personal fact to a home-directory import makes it an external import the user must approve once, and a decline leaves it disabled with no second dialog; and a line that exists only to correct a model default carries its default into the handoff.
- **`references/skills.md` drops its client-owned values and one claim that had no source.** The listing budget fraction, the description character cap and the compaction token budgets are replaced by what a generator decides from: the skills invoked least are the first to lose their descriptions, so adding a skill can silence a different one nobody edited; a trailing clause in a description is the expendable part; and an older skill's body can be gone entirely while the session continues, which is why a behaviour a long session cannot lose is backed in CLAUDE.md. **"Current models under-trigger" is gone** — an undated claim about model behaviour that no live doc supports, and the same class as the six wrong gotchas this skill has shipped. The advice it justified survives on the asymmetry alone, which needs no evidence beyond itself: a dark skill costs the whole skill, a marginal load costs some context. Three phrases the file attributed to `SKILL.md` had not existed there since the v8 rewrite; they are rewritten in the file's own voice.
- **`references/agents.md` reaches for `effort:` before a cheaper tier, and drops a version marker.** When the reason to pin `model` is that the role needs more or less thinking than the parent gives it, that is what `effort:` is for and it leaves the parent's model in place; the tier changes when the need is the model itself. The workspace-trust gotcha loses `(v2.1.218+)`: the version says only when the gate started, which changes nothing a generator decides, since the agent's frontmatter hooks are inert until the folder is trusted either way. Two overstatements went with it — the debug-log entry is what explains the skip rather than the only trace of it.
- **`references/workflows.md` loses its concurrency numbers, its version floor, and a wrong entitlement claim.** The caps stay as what they are — backstops, not a budget — with the decision they actually drive: a legitimately large workload can exceed one run's total, and the answer is to group units per agent or split runs, decided from the unit count before launching. The gating bullet said workflows require a paid plan; the live docs also list Anthropic API access and three cloud providers, so a harness pass could have refused to generate one for an eligible user. `"Conviction over compliance"`, quoted here from a `SKILL.md` that has not contained it since v8, is rewritten in the file's own voice.
- **`references/e2e-testing.md` stops restating `SKILL.md`.** The file carried no client-owned values, so this pass was density only: three sentences that repeated the orchestrator — the structural-versus-behavioural split, choose-your-coverage, and use-the-serving-model — are gone, and what each was introducing stays. The coverage sentence now says the part only this file owns, that the kinds pull in different directions and a set holding only intended triggers is short, with the boundary cases scoped to designs that have a boundary.
- **`references/hooks.md` loses every version marker and every limit value, and three inherited inaccuracies with them.** The `Stop` block cap, the timeout figures and six release floors are gone; what decides anything stays — check `stop_hook_active` regardless of the cap, keep slow work out of the events sitting in a path the user waits on, and comment the floor where a gated feature is used, looked up rather than carried. Exit codes `0`/`1`/`2` deliberately **stay**: they are the identity of the mechanism, and the gotcha that only exit 2 blocks while exit 1 proceeds silently cannot be stated without them. The corrections: "three events depart" had gone stale the same way "two events" did before it, so the count is gone and a test now forbids a new one; `SessionEnd` has a per-hook default *and* a shared budget, not a budget instead of a default; and project-level `defaultMode: "auto"` does not fall back one level but drops to the built-in default, skipping any `defaultMode` the user set for themselves.
...
- **`test_hook.py`'s own example produced a false PASS.** `--input-field command="rm -rf /"` sets a top-level key, while a hook reads its tool arguments from `tool_input` — so following the documented example left `tool_input` at its sample value and tested the hook against `echo hello`. A protected-path guard reported PASS having inspected nothing that was asked for, which is the one outcome a hook tester must never produce. The top-level behaviour is deliberate and stays; the example and `--input-field`'s help now show the object form that reaches the hook, and three tests pin it, including one asserting the example itself. Found by exercising the reference's own Recipe 1 during the `hooks.md` audit.
- **`CLAUDE.md` pointed into `docs/plan/`, which no longer exists**, for both the binding spec and the source to verify Claude Code mechanics against. The record is the changelog and the commits behind it. Verification now names the live docs and requires a verbatim sentence, treating "no such sentence exists" as an answer — the `.tmp/docs_claude/` snapshot the old rule named is how six wrong gotchas shipped.
- **`tools/gotcha-quiz.jsonl`'s `source` line numbers** no longer resolve against the references and are labelled as the provenance they are: `probe.py` echoes the field and never resolves it, and the quiz stays frozen so an earlier run's numbers keep their meaning.
