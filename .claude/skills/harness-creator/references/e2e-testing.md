...
This is inherited directly from skill-creator's grader — the single most important habit to carry over, because it's the difference between an e2e tier that catches real problems and one that produces reassuring-looking JSON that means nothing.
...
State this plainly to the user the first time you propose running e2e for real, in these words or close to them: headless `-p` mode's permission handling — which combination of `--permission-mode`, `--dangerously-skip-permissions`, and pre-registered `permissions.allow` entries actually lets a scenario run to completion without stalling — is this file's best documented guess, not a confirmed fact. The environment that built this harness-creator skill could not authenticate a spawned `claude` child process at all (spawned processes returned "Not logged in" even for a simple headless call, because the host session's OAuth/keychain credentials don't propagate to a Bash-spawned child), so `run_e2e.py`'s actual invocation of `claude -p` was built from `.tmp/docs_claude` documented behavior and has never itself been watched succeed end to end.
...
Description auto-tuning via a train/test holdout loop — skill-creator's "System B," which iteratively rewrote a skill's description against a held-out query set to improve trigger precision — is out of scope for v1. If trigger quality turns out to be a recurring problem across multiple harness-creator runs rather than a one-off fix, it's a reasonable candidate to reintroduce later; it is not needed to ship v1's e2e tier.
