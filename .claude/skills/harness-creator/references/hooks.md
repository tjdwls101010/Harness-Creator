...
**Exit 2 outranks the JSON, and the JSON still gets read.** These are two channels, not two mutually exclusive ones: valid JSON on stdout is parsed whatever the exit code, and on an event that can block, exit 2 blocks anyway — a JSON `permissionDecision: "allow"` cannot talk it out of blocking. What exit 2 overrides is the JSON's allow/block *outcome*, not the object: the rest of a valid payload is still honored, and the blocking message is the reason from a blocking decision if your JSON made one, your stderr otherwise. So write one shape per hook and know which one you are in — "exit 0, print a decision object" for anything richer than a block (allow/deny/ask, `updatedInput`, `additionalContext`), or "exit 2 with the reason on stderr" for a hard block — and don't reach for exit 2 to *strengthen* a JSON decision, which only overrules the decision you just wrote. (Two exceptions worth knowing: on `Elicitation` and `ElicitationResult` an exit-2 hook's `hookSpecificOutput` is ignored, and JSON that fails schema validation still blocks on exit 2, falling back to stderr with the validation failure recorded only in the debug log.)
...
**Default timeouts are wildly uneven, and mismatched expectations cause silent truncation.** `command`, `http` and `mcp_tool` share a default generous enough that people stop thinking about it — and then a handful of events cut it sharply, other handler types carry their own, and one event departs in kind rather than in magnitude. So the figure is per event and per handler type, and the one you remember is the one that bites: look it up with `hook_event.py --event <Event>` when the hook does real work.

What the pattern tells you without any figure is where a short one is waiting. An event sitting in the path of something the user is waiting on gets cut: `UserPromptSubmit` blocks every prompt from reaching Claude, `MessageDisplay` sits in the render path. Teardown is cut hardest, and `SessionEnd` is also the one whose **mechanism** differs: on top of the usual per-hook default it has an overall budget shared across every `SessionEnd` hook, which no other event has. Raising one hook's `timeout` raises that shared budget to match, up to a ceiling, and `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` overrides it; a timeout set on a *plugin-provided* hook does not raise it at all.

So a `SessionEnd` hook that shells out to anything slow gets truncated during teardown, and a second `SessionEnd` hook eats into the same budget rather than getting its own. Treat teardown as a place to write a line, not to run a job.
...
**Several features are version-gated; generate for latest stable and comment the floor.** The `if` field, comma-separated matcher lists, hyphens inside an exact-match matcher and `defer` each arrived in a particular release. Assume latest stable and add a one-line comment naming the floor where you use a gated feature — hedging every hook against every older version produces configuration nobody can read. Look the floor up when you write that comment rather than carrying one in your head: `hook_event.py --event <Event>` reports the caveats attached to an event, and the matcher-syntax floors are stated in the hook documentation beside the syntax they gate. The comment is worth writing because at least one of these degrades silently rather than being rejected: before hyphens were exact-matchable, a matcher like `code-reviewer` became an unanchored regex that also fired on `senior-code-reviewer` — a hook that runs, on the wrong things, which is the failure a version note on the matcher line would have caught.
...
- **The exact folder must be trusted.** `permissions.allow` and `additionalDirectories`, a project subagent's frontmatter hooks (the agent still runs, hookless), a repository `@skills-dir` plugin, `extraKnownMarketplaces`, and a subagent's inline `mcpServers`. Trusting a parent does not reach a nested project here.
...
A repository cannot grant itself auto mode. Claude Code **ignores `permissions.defaultMode: "auto"`** when it appears in `.claude/settings.json` or `.claude/settings.local.json`, with no error and no warning. The session does not merely fall back one level: it drops to the built-in default and skips any `defaultMode` the user set in their own `~/.claude/settings.json`, so writing this into a project can leave a developer in a *different* mode than they had before the file existed. Only `~/.claude/settings.json` (or managed settings) can set it.
