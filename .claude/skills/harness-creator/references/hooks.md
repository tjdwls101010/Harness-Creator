...
**Exit 2 outranks the JSON, and the JSON still gets read.** These are two channels, not two mutually exclusive ones: valid JSON on stdout is parsed whatever the exit code, and on an event that can block, exit 2 blocks anyway — a JSON `permissionDecision: "allow"` cannot talk it out of blocking. What the two of them decide together is only the *message*: the reason from a blocking decision in your JSON if it made one, and your stderr otherwise. So write one shape per hook and know which one you are in — "exit 0, print a decision object" for anything richer than a block (allow/deny/ask, `updatedInput`, `additionalContext`), or "exit 2 with the reason on stderr" for a hard block — and don't reach for exit 2 as a way to *strengthen* a JSON decision, since it discards every part of that decision except its reason. (Two exceptions worth knowing: on `Elicitation` and `ElicitationResult` an exit-2 hook's `hookSpecificOutput` is ignored, and JSON that fails schema validation still blocks on exit 2, falling back to stderr with the validation failure recorded only in the debug log.)
