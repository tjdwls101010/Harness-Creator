...
This file enumerates all 30 hook events Claude Code supports. It is the data `hook_event.py` reads, and taking one event's worth with `hook_event.py --event <Event>` is how to consult it — reading the whole file costs about 3,800 words to answer a question about one event. For picking the right event in the first place, use the one-line router table in `references/hooks.md`, which also carries the design principles and gotchas that apply regardless of event. This file is pure lookup: trigger timing, matcher support, input fields, decision channel, typical use, and version caveats, per event.
...
| `SessionEnd` | The session terminates. | `clear`, `resume`, `logout`, `prompt_input_exit`, `bypass_permissions_disabled`, `other` (the `reason`) | `reason` | None — can't block termination. | Cleanup tasks, session-statistics logging, saving state. | **1.5-second budget shared across all `SessionEnd` hooks** — not a per-hook default, unlike every other event. Auto-raised to the highest per-hook `timeout` configured in settings (plugin-provided hooks don't raise it), up to 60s; override via `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS`. |
