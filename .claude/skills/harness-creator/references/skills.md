...
**What compaction keeps of a skill is budgeted, and the budget is spent newest-first.** The listing survives — every skill's name is still there to trigger on. What is rationed is the *body* of a skill that already ran: auto-compaction re-attaches the most recent invocation of each one, keeping the first 5,000 tokens of each within a combined 25,000, filled from the most recently invoked backwards. So in a session that invoked several skills, the earliest one's instructions can be gone while the model still believes it is following them, and nothing announces the drop. For a behaviour a long session cannot afford to lose, back it with a one-line rule in CLAUDE.md, which is re-read from disk after compaction — or tell the user to re-invoke the skill, which restores it in full.
