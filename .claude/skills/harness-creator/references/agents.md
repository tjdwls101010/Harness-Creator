...
Both push one way: a harness that depends on those fields is distributed as a repo `.claude/` tree rather than as a plugin. A workflow is not in that set — it packages. Ask whether the harness must travel (SKILL.md's K11) before routing a role, so this is a design input rather than a discovery.
