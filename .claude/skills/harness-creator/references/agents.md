...
Agent count is itself a cost, independent of any single agent's quality: each addition is one more role the orchestrating Claude has to keep in mind when routing a task, one more `description` competing for the same routing attention, one more definition that can silently rot out of sync with the codebase. A harness with twelve narrowly-scoped agents is not more capable than one with three well-chosen ones — it's more expensive to reason about and more likely to misroute. Known antipattern: a meta-harness that makes "Agent Teams, 4-5 agents" the unconditional default architecture for every generated harness, regardless of what the target project actually needs — a travel-planning harness getting the same five-agent shape as an incident-postmortem harness. Generate a role only when the interview has produced a concrete, demonstrated need for it — a specific recurring task, a specific volume-of-output problem, a specific tool-restriction requirement — never as a default shape reached for because "harnesses have agents."
