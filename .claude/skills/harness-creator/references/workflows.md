...
- **Pre-approve what a workflow's agents need, because a prompt is the one thing that stops the run.** They use your permission rules and resolve their mode the way any subagent does, so an agent can land on a permission prompt — and an agent permission prompt is the only thing that can pause a run, which is how an unattended one ends up stopped. So if a fan-out runs `npm test` or fetches a URL, generate the matching `permissions.allow` entries in the same pass and the same commit, treat them as one deliverable, and record in the spec's Design rationale why they exist: on disk they read as ordinary convenience allowances, and the next person to tighten permissions deletes them and leaves a workflow that hangs. The launch itself is a tool call too, and in `claude -p` and the SDK there is no dialog to approve it — a `Workflow` allow rule (or `Workflow(<name>)` for one saved workflow) is the plainest way to let it start there, though auto mode's classifier, bypass mode, a `PreToolUse` hook returning allow, or the host's own approval callback each do it as well.
