...
- **Concurrency and run size have caps, and they are backstops rather than a budget.** Size the fan-out to the real unit count — one agent per file, per route, per source — rather than to the cap. The caps are not only a net, though: a legitimately large workload can exceed what one run allows in total, and the answer is to group several units into one agent or split the work across runs. Decide that from the unit count before launching, not from a run that stopped partway.
