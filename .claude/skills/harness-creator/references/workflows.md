...
- **A workflow's agents get no interactive prompt, so anything they need has to be allowed before the run starts.** They use your permission rules, and their permission mode follows the ordinary subagent rules rather than the invoking session's — which is why "it works when I do it by hand" is no evidence about the run. Nothing is waiting mid-run to grant a permission: the agent stalls on a prompt nobody is watching. So if a workflow's fan-out runs `npm test` or fetches a URL, generate the matching `permissions.allow` entries in the same pass and the same commit, treat them as one deliverable, and record in the spec's Design rationale why they exist — on disk they read as ordinary convenience allowances, and the next person to tighten permissions deletes them and leaves a workflow that hangs. The launch itself is a tool call too: under `claude -p` and the SDK there is no approval dialog at all, so a `Workflow` allow rule (or `Workflow(<name>)` for one saved workflow) is what lets it start there.
