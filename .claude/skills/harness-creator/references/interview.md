...
**Say out loud what you are choosing not to build, and record it.** Every layer in this framework costs something on every session — a CLAUDE.md line, a skill's description sitting in the listing budget, an agent adding a routing decision — so a harness is defined as much by what it leaves out as by what it contains. When you decline a candidate, name it and give the reason in one line ("skipping a skill for changelog entries: it's two commands, and a CLAUDE.md line covers it"), then write it into the inventory table with `status: declined`. A declined row is worth its space: without it the next pass re-proposes the same idea and the user has to re-decide something they already decided, and there's no record of *why* the harness looks the way it does. This is also the moment to fold near-duplicate candidates together, rather than discovering the overlap after three skills already exist.
