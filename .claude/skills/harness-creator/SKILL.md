...
Write for the model using the harness. Keep the **gotcha** that general competence cannot supply: a silent failure, an invisible project decision, a boundary the model would otherwise misread. The history of discovering it belongs in development records unless that history changes the next execution's judgment. A paragraph in a reference must change what the builder writes or does.

Density means preserving the knowledge needed to decide while removing work the reader need not do. Keep the clause that makes the rule re-derivable; cut restatements, arguments that merely defend it, consequences the reader can compute, and narration of what comes next. Group related conditions with the judgment they qualify rather than compressing unrelated obligations into one paragraph. Where a literal phrasing of the wanted behavior exists, write that phrasing instead of a description of it. A useful example or necessary exception is not expendable to meet a word count.
...
Carry the client behavior the model cannot observe while it runs: an instruction truncated out of the context it was written for, an unscoped rule loading into every session, a malformed frontmatter that leaves a skill invocable by name while its trigger is quietly dead. Write it as the failure shape that changes a placement decision, not as the limit value behind it. Leave out what the model can read for itself and the values the client owns: a release moves them while the prose goes on asserting the old one, and a number on the page answers once, for the whole file, a question that belongs to each line — whether the model gets *this* one wrong when it is gone. A number the project itself chooses is the opposite case and stays: a threshold or a retry count carries its reason and the conditions under which another value is right.

A line written against a model default is the most useful line in a harness and the one whose expiry is tied to the model. A rule against bullet lists, written for a model that over-formatted, pushes a model that already under-formats further the wrong way; "save the findings for the end" silences a model that was terse to begin with. Write such a line only when you can name the default it corrects, record that default where the next change will find it, and treat a model change as the event that reopens every line that names one.
...
A handoff explains the change on the project's delivery surface; it is not another harness component to generate. The diff shows what changed; the handoff supplies the decisions that explain it, the evidence behind them, and what would reopen them. If current files contradicted a past decision, record how that disagreement was resolved. A reversal names the decision it supersedes, since immutable history cannot be updated in place. An instruction written to correct a model default records which default, because a model change can leave it still needed, merely redundant, or pushing the wrong way, and nothing in the file distinguishes the three — that is what the removal test above is for.
