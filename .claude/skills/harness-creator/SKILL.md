...
- **K2.** On an empty repo ask for goals and pain points; on an existing harness ask what should change and, in the same breath, **what is now unnecessary** — a harness only grows and there is no invocation telemetry, so nobody asks unless you do; "nothing comes to mind" is a real answer.
...
- **K11.** Ask once whether any of this must work beyond this repo, and record the answer in the spec — skills, agents and hooks can be packaged, CLAUDE.md, rules and permissions have no portable form, and no documented way exists for a plugin to ship a workflow.
- **K12.** Ask what language the generated harness should be written in — it may differ from the language of the interview.
- **K13.** Diverge in conversation, converge with AskUserQuestion, and make each option's description the reason to pick it rather than its label restated — that is what lets the user judge. Read the user's vocabulary from their first answer and keep speaking it.
- **K14.** Offer e2e only after stating its cost and getting consent — it spends real tokens and time.
...
| A procedure or playbook needed only when a job comes up | skill | Triggers on `description`; the body loads only then. Skill count is a cost (references/skills.md), so consolidate in the inventory, not after | Doesn't trigger or steals triggers: frontmatter parse → listing budget → `disable-model-invocation` → compaction → wording, in that order. Wrong behaviour after triggering: the body's *why* |
| Something that must (or must never) happen every time | hook, paired with a `permissions` rule | Advisory layers can be deviated from; a hook fires regardless. Pair it because a hook's own `if` filter fails open (references/hooks.md) | Doesn't fire: reproduce with `test_hook.py` first. Over-fires: narrow the matcher, or downgrade to a warning if the cost never justified blocking |
| A tool, command or path to block or force-approve | `permissions.allow` / `permissions.deny` | Enforced by the client, independent of the model | An allow rule never fires: a deny somewhere matched first; carve the exception out of the deny |
...
| `validate_harness.py` | After every generation or edit, and again after the wrap-up edits. Exit 0 is finished; it checks shape, never behaviour |
| `hook_event.py` | Once you know which event you target, instead of reading all thirty |
| `test_hook.py` | After generating or wiring any hook, before calling it delivered. Its verdict approximates a session; the session is final |
| `run_e2e.py` | Only with the user's consent, during validation: it spends real tokens. Isolate anything that writes; auth is per-machine, so a first run on a new machine is itself the confirmation |

`validate_harness.py` cannot grade a `description` for trigger quality or sibling overlap; re-read every description you generate against references/skills.md, a lone one included. Every pointer this skill or its output writes must resolve to a real file: the check catches the ones written with a `references/` or `scripts/` prefix, a bare filename is invisible to it, so it is a floor, not a substitute for looking.
