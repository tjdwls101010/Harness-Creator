...
- **K7.** Settle the design in dependency order, approving each piece before the next depends on it; pieces whose inputs are already settled can go together. Keep "is this worth having" apart from "where does it live" — different questions, and mixing them makes both harder to judge.
...
- **K16.** The handoff carries what the diff cannot: the decisions that change the next pass's judgement, and what would reopen them. Two are invisible by construction — what you decided *not* to build and why, and any checkpoint the user approved away (K10). Put them in the commit body rather than only the pull request; a squashed history and an offline clone keep one and not the other. A decision that changed nothing on disk has no diff to attach to: say so and ask where it should go.
