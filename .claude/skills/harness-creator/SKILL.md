...
| Something that must (or must never) happen every time | hook; a must-never `PreToolUse` gets a `permissions.deny` pair | Advisory layers can be deviated from; a hook fires regardless. The pair, because a hook's `if` filter fails open; `PostToolUse` and `Stop` have no rule expressing the same guarantee (references/hooks.md) | Doesn't fire: reproduce with `test_hook.py` first. Over-fires: narrow the matcher, or downgrade to a warning if the cost never justified blocking |
