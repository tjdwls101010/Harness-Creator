...
**The four-frame doctrine has exactly one canonical statement, in `SKILL.md`.** It had four: `skills.md` restated the litmus test at greater length than the original, `claude-md-and-rules.md` restated the enforced-vs-advisory question that `hooks.md` also asks, and `workflows.md` quoted "the one sentence that governs every layer" without citing it. Each reference now keeps only its component-specific application and anchors to the canonical phrase. The drift this prevents was already measurable: `skills.md` said a body over 400 lines earns a second file while the check said 500.
...
- **2026-08-24 — improve (v6).** Applied the skill's own four frames to itself for the first time as a whole, rather than establishing one more.
  ...
  - Verified by two adversarial `gpt-5.6-sol` passes. The first returned 23 findings against the plan and changed eight decisions, including two mis-citations of this repo's own history and one defect the audit had missed.
